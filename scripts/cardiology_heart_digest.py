    ...
    try:
        headers = {
            "User-Agent": (
                ...
                "+https://github.com/Meggers1982/cardiology-heart-digest)"
            ),
...
{{
  ...
  "groundbreaking": "one or more of: Counterintuitive finding / Overturns prior research / First-in-class human study / Major cardiovascular or public health implication",
  "media_coverage": "{media_note}",
  "summary": "2-4 sentences: what researchers did, who participated (N=X, age range, cardiac history if relevant), key finding in plain language — verified against abstract",
  "why_it_matters": "1-2 sentences of real-world significance for people concerned about heart health. Do NOT imply clinical action.",
  "caveats": "comma-separated flags: small sample (N<100), observational design, single-center, self-reported outcomes, short follow-up, industry funding [name], preprint, secondary analysis, surrogate endpoints only, animal or cell study — or 'None identified'",
  "fact_check_note": "Describe any corrections made vs what the abstract actually says, or empty string",
  "excluded": false,
  "relevance_score": 7,
  "relevance_score_reason": "One sentence explaining the score — what makes it strong or weak for cardiovascular journalism",
  "pitch_angles": [
    {{
      ...
      "pitch_angle": "2-3 sentence commissioning pitch: what happened, why it matters for heart health, broader lifestyle or clinical significance",
...
- Generate MULTIPLE angles (2-3 max) only when the study genuinely fits different audiences with meaningfully different framings — e.g. a study on heart failure and sleep apnea could pitch differently to STAT News (clinical mechanism angle) vs Prevention (lifestyle/risk reduction angle) vs AARP (aging and heart health angle)
...
- Distinguish between surrogate endpoints (cholesterol, blood pressure, biomarkers) and hard endpoints (heart attack, stroke, death) — surrogate-only findings require explicit caveats
- For hematology: specify the condition (DVT, PE, anemia, clotting disorders, etc.) — do not use vague terms
- For pulmonary medicine: note whether the condition is acute (pneumonia, PE) or chronic (COPD, asthma, pulmonary hypertension) and the study context
- Sex differences in cardiovascular disease are significant — flag if a study lacks female-specific analysis, especially for heart attack presentation or treatment outcomes
...
relevance_score rubric (1–10): start at 5, then adjust:
  ...
  +2 human subjects with hard cardiovascular endpoints (N≥100)
  +1 clear lifestyle hook (diet, exercise, sleep, stress, smoking)
  +1 clean design (RCT, large prospective cohort, meta-analysis)
  +1 high-prevalence condition (coronary artery disease, heart failure, hypertension, AFib, stroke)
  −1 per major caveat
...
{studies_block}"""
    ...
    try:
        ...
        return [{"pmid": s["pmid"], "headline": s["title"], "journal": s["journal"],
                 ...
                 "groundbreaking": "Relevant heart health finding", "media_coverage": media_note,
...
def send_notification(category: str, chunk_label: str, study_count: int, run_date: str):
    subject = f"Cardiology & Heart Health Research Digest — {run_date} | {study_count} {'Study' if study_count == 1 else 'Studies'}"
    html = f"""<!DOCTYPE html>
<html>
<body style="font-family:Georgia,serif;max-width:500px;margin:auto;padding:24px;color:#222;">
<h2 style="color:#1a3a2e;">Cardiology &amp; Heart Health Research Digest</h2>
...
<p style="font-size:0.85em;color:#888;margin-top:2em;">
  Cardiology &amp; Heart Health Research Digest · PubMed + Claude + SERPAPI
</p>
</body>
</html>"""
    try:
...
def main():
    ...
    print("=" * 60)
    print("Cardiology & Heart Health Research Digest")
